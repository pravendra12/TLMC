...
namespace api
{
  ...
  void Parameter::ReadParam(const std::string &param_filename)
  {
    ...
    while (std::getline(ifs, buffer))
    {
      ...
      else if (segs[0] == "cube_size")
      {
        cube_size_ = stoul(segs[1]);
      }
      else if (segs[0] == "vacancy_lattice_id")
      {
        vacancayLatticeId_ = stoul(segs[1]);
      }
      else if (segs[0] == "small_config_id")
      {
        smallConfigId_ = stoul(segs[1]);
      }
