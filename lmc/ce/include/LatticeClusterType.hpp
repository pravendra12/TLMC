...
public:
  ...
  {
    if (lhs.bond_order_vector_.size() != rhs.bond_order_vector_.size())
    {
      return false;
    }
    for (size_t i = 0; i < lhs.bond_order_vector_.size(); ++i)
    {
      if (lhs.bond_order_vector_[i] != rhs.bond_order_vector_[i])
      {
        return false;
      }
    }
  ...
  friend bool operator<(const LatticeClusterType &lhs, const LatticeClusterType &rhs)
  {
    ...
    for (size_t i = 0; i < lhs.bond_order_vector_.size(); ++i)
    {
      ...
      if (rhs.bond_order_vector_[i] < lhs.bond_order_vector_[i])
      {
        return false;
      }
    }
