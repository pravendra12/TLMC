...
public:
  // Constructor
  TiledSupercell(
      const Config &smallConfig, const Cube &cubeObj);
  ...
  vector<pair<size_t, int>> GetNeighborLatticeIdVectorOfLattice(
   ...
   * the supercell.
   */
  pair<size_t, size_t> GetLatticeAndConfigIdFromAtomId(
...
private:
  ...
  vector<vector<vector<pair<size_t, int>>>> neighbourList_{};
