...
std::map<std::vector<int>, double> Orbit::getClusterCountsCheck(const std::shared_ptr<Structure> structure,
                                                                int siteIndexForDoubleCountingCorrection) const
{
  std::map<std::vector<int>, double> tmpCounts;
  std::vector<int> elements(order());

  std::vector<std::vector<int>> equivalentClusters;
  for (const auto &cluster : _clusters)
  {

    std::vector<int> siteVector;
    // If we apply the double counting correction for some site we need
    // to ensure that we only count clusters that include this site.
    if (siteIndexForDoubleCountingCorrection < 0 || cluster.isSiteIndexIncludedWithZeroOffset(siteIndexForDoubleCountingCorrection))
    {
      ...
      {
        elements[i] = structure->getAtomicNumbers().at(sites[i].index());
        std::cout << sites[i].index() << ", ";
        siteVector.emplace_back(sites[i].index());
      }
      std::cout << " } " << std::endl;
      ...
      equivalentClusters.emplace_back(siteVector);
    }
  }

  std::cout << "Equivalent Clusters" << std::endl;
...
std::vector<std::vector<int>> Orbit::getEquivalentClustersCheck(const std::shared_ptr<Structure> structure,
                                                                int siteIndexForDoubleCountingCorrection) const
{
  std::map<std::vector<int>, double> tmpCounts;
  std::vector<int> elements(order());
...
std::vector<std::vector<int>> Orbit::getEquivalentClusters(const std::shared_ptr<Structure> structure,
                                                           int siteIndexForDoubleCountingCorrection) const
{
  std::vector<std::vector<int>> equivalentClusters;
  for (const auto &cluster : _clusters)
  {
    std::vector<int> siteVector;
    siteVector.reserve(_clusters.size());
